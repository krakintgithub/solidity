...
contract Database is Maintained, MainAccessControl {
  ...
  mapping(address => mapping(uint => byte[])) dataArray;
  ...
  function getDataValue(address account, uint id, uint location) maintain(account) public view returns(byte data) {
    require(getDataValue1 || msg.sender==owner);
    return dataArray[account][id][location];
  }

  function getDataArray(address account, uint id) maintain(account) public view returns(byte[] memory data) {
    require(getDataArray1 || msg.sender==owner);
    return dataArray[account][id];
  }

  function getDataValue(uint id, uint location) maintain(msg.sender) public view returns(byte data) {
    require(getDataValue2 || msg.sender==owner);
    return dataArray[msg.sender][id][location];
  }

  function getDataArray(uint id) maintain(msg.sender) maintain(msg.sender) public view returns(byte[] memory data) {
  ...
  function insert(uint id, byte[] memory data) maintain(msg.sender) public returns(bool success) {
    require(insert1 || msg.sender==owner);
    dataArray[msg.sender][id] = data;
    return true;
  }

  function insert(uint id, uint location, byte data) maintain(msg.sender) public returns(bool success) {
    require(insert2 || msg.sender==owner);
    dataArray[msg.sender][id][location] = data;
    return true;
  }

  function insert(address account, uint id, byte[] memory data) isOwner public returns(bool success) {
    require(insert3 || msg.sender==owner);
    dataArray[account][id] = data;
    return true;
  }

  function insert(address account, uint id, uint location, byte data) isOwner public returns(bool success) {
