...
contract Database is Maintained, MainAccessControl {
  ...
  function getDataValue(address account, uint id, uint location) maintain(account) public view returns(bytes32[] memory data) {
    require(getDataValue1 || msg.sender==owner);
    return dataArray[account][id][location];
  }

  function getDataValue(uint id, uint location) maintain(msg.sender) public view returns(bytes32[] memory data) {
  ...
  function insert(uint id, uint location, bytes32[] memory data) maintain(msg.sender) public returns(bool success) {
    require(insert2 || msg.sender==owner);
    dataArray[msg.sender][id][location] = data;
    return true;
  }

  function insert(address account, uint id, uint location, bytes32[] memory data) isOwner public returns(bool success) {
